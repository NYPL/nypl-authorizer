...
function validateScopes (request, Config) {
  return new Promise((resolve, reject) => {
    ...
    request.getRequiredScopes().forEach(requiredScopeSet => {
      ...
      if (!scopeSetValid) return reject(new Errors.InvalidScopeValidatorError(`Required scope set (${requiredScopeSet}) was not found in token scope (${request.getScopes()})`))
